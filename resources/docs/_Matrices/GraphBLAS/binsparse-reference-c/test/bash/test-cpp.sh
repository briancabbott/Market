...
convert_ssmc() {
  ...
  echo "mtx2bsp-cpp ${main_matrix} ${dest_file} ${format}"
  mtx2bsp-cpp ${main_matrix} ${dest_file} ${format}
  ...
  do
    ...
    echo "mtx2bsp-cpp ${secondary_matrix} ${dest_file}:${secondary_name}"
    mtx2bsp-cpp ${secondary_matrix} ${dest_file}:${secondary_name}
  done
...
check_ssmc() {
  ...
  check_equivalence-cpp ${main_matrix} ${dest_file}
  ...
  do
    ...
    secondary_name=${secondary_name/.mtx/}
    secondary_name=${secondary_name/${matrix_name}_/}
    echo "check_equivalence-cpp ${secondary_matrix} ${dest_file}:${secondary_name}"
    check_equivalence-cpp ${secondary_matrix} ${dest_file}:${secondary_name}
  done

  rm -r ${directory}/${matrix_name}
}

export PATH=$PATH:$PWD/../../examples/cpp
